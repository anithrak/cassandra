...
try:
    ...
    pass
if webbrowser._tryorder and webbrowser._tryorder[
    0] == 'xdg-open' and os.environ.get('XDG_DATA_DIRS', '') == '':
...
if is_win and sys.version_info < (3, 3):
    codecs.register(
        lambda name: codecs.lookup(UTF8) if name == CP65001 else None)
...
parser.add_option("--encoding",
                  help="Specify a non-default encoding for output." +
                       " (Default: %s)" % (UTF8,))
parser.add_option("--cqlshrc",
                  help="Specify an alternative cqlshrc file location.")
parser.add_option('--cqlversion', default=DEFAULT_CQLVER,
                  help='Specify a particular CQL version (default: %default).'
                       ' Examples: "3.0.3", "3.1.0"')
parser.add_option("-e", "--execute", help='Execute the statement and quit.')
parser.add_option("--connect-timeout", default=DEFAULT_CONNECT_TIMEOUT_SECONDS,
                  dest='connect_timeout',
                  help='Specify the connection timeout in seconds (default: %default seconds).')
parser.add_option("--request-timeout", default=DEFAULT_REQUEST_TIMEOUT_SECONDS,
...
def complete_help(ctxt, cqlsh):
    return sorted([t.upper() for t in
...
def complete_copy_column_names(ctxt, cqlsh):
    existcols = list(
        map(cqlsh.cql_unprotect_name, ctxt.get_binding('colnames', ())))
...
COPY_FROM_OPTIONS = ['CHUNKSIZE', 'INGESTRATE', 'MAXBATCHSIZE', 'MINBATCHSIZE',
                     ...
                     'MAXINSERTERRORS', 'ERRFILE', 'PREPAREDSTATEMENTS']
...
def extend_cql_deserialization():
    ...
    cassandra.cqltypes.BytesType.deserialize = staticmethod(
        lambda byts, protocol_version: bytearray(byts))
    ...
    def deserialize_date_fallback_int(byts, protocol_version):
        ...
        try:
            ...
            warnings.warn(DateOverFlowWarning(
...
def auto_format_udts():
    ...
    cassandra.cqltypes.UserType.udt_apply_parameters = classmethod(
        new_apply_params)
...
class Shell(cmd.Cmd):
    ...
    def is_using_utf8(self):
        ...
        return self.encoding.replace('-', '_').lower() in ['utf', 'utf_8', 'u8',
                                                           'utf8', CP65001]
    ...
    def show_version(self):
        ...
        print(
            "[cqlsh %(shver)s | Cassandra %(build)s | CQL spec %(cql)s | Native protocol v%(protocol)s]" % vers)
    ...
    def get_columnfamily_names(self, ksname=None):
        ...
        return list(
            map(str, list(self.get_keyspace_meta(ksname).tables.keys())))
    ...
    def get_index_names(self, ksname=None):
        ...
        return list(
            map(str, list(self.get_keyspace_meta(ksname).indexes.keys())))
    ...
    def get_usertype_layout(self, ksname, typename):
        ...
        return [(field_name, field_type.cql_parameterized_type())
                ...
                zip(user_type.field_names, user_type.field_types)]
    ...
    def handle_statement(self, tokens, srcstr):
        ...
        return self.perform_statement(
            cqlruleset.cql_extract_orig(tokens, srcstr))

    def is_forbidden_statement(self, tokens):
        # Checks whether the statement mutates data in restricted tables

        default_restricted_tables = [
            'dc_blob_store_group',
            'sharded_chain_blob_store_group',
        ]
        comma_sep_restricted_tables = os.getenv('CQLSH_RESTRICTED_TABLES')
        if comma_sep_restricted_tables is not None:
            restricted_tables = comma_sep_restricted_tables.split(',')
        else:
            restricted_tables = default_restricted_tables

        cmdword = tokens[0][1]
        if str(cmdword.lower()) not in ('insert', 'update', 'delete'):
            return False, None
            ...
            if self.tracing_enabled:
                try:
                    for trace in future.get_all_query_traces(
                            max_wait_per=self.max_trace_wait,
    ...
    def perform_simple_statement(self, statement):
        ...
        if not future.is_schema_agreed:
            try:
                self.conn.refresh_schema_metadata(
                    5)  # will throw exception if there is a schema mismatch
            except Exception:
                self.printerr(
        ...
        if statement.query_string[:6].lower() == 'select':
            self.print_result(result, self.parse_for_select_meta(
                statement.query_string))
        elif statement.query_string.lower().startswith(
                "list users") or statement.query_string.lower().startswith(
                "list roles"):
            self.print_result(result,
                              self.get_table_meta('system_auth', 'roles'))
    ...
    def print_static_result(self, column_names, rows, table_meta):
        ...
        if not rows:
            ...
            return
        formatted_values = [list(map(self.myformat_value, list(row.values())))
                            for row in rows]
    ...
    def print_formatted_result(self, formatted_names, formatted_values):
        ...
        for row in formatted_values:
            line = ' | '.join(
                col.rjust(w, color=self.color) for (col, w) in zip(row, widths))
        ...
        if ksname is None:
            for ksmeta in self.get_keyspaces():
                ...
                cmd.Cmd.columnize(self,
                                  protect_names(list(ksmeta.functions.keys())))
                print('')
        else:
            ksmeta = self.get_keyspace_meta(ksname)
            cmd.Cmd.columnize(self,
    ...
    def describe_function(self, ksname, functionname):
        ...
        print('')
        ksmeta = self.get_keyspace_meta(ksname)
        functions = [f for f in list(ksmeta.functions.values()) if
                     f.name == functionname]
        if len(functions) == 0:
    ...
    def describe_aggregates(self, ksname):
        ...
        if ksname is None:
            for ksmeta in self.get_keyspaces():
                ...
                print('---------%s' % ('-' * len(name)))
                cmd.Cmd.columnize(self,
                                  protect_names(list(ksmeta.aggregates.keys())))
                print('')
        else:
            ksmeta = self.get_keyspace_meta(ksname)
            cmd.Cmd.columnize(self,
    ...
    def describe_aggregate(self, ksname, aggregatename):
        ...
        print('')
        ksmeta = self.get_keyspace_meta(ksname)
        aggregates = [f for f in list(ksmeta.aggregates.values()) if
                      f.name == aggregatename]
        if len(aggregates) == 0:
            raise FunctionNotFound(
    ...
    def describe_usertypes(self, ksname):
        ...
        if ksname is None:
            for ksmeta in self.get_keyspaces():
                ...
                print('---------%s' % ('-' * len(name)))
                cmd.Cmd.columnize(self,
                                  protect_names(list(ksmeta.user_types.keys())))
                print('')
        else:
            ksmeta = self.get_keyspace_meta(ksname)
            cmd.Cmd.columnize(self,
    ...
    def describe_cluster(self):
        ...
        if self.current_keyspace is not None and self.current_keyspace != 'system':
            ...
            for entry in list(ring.items()):
                print(' %39s  [%s]' % (str(entry[0].value), ', '.join(
                    [host.address for host in entry[1]])))
            print('')
  ...
  """
    ...
    def do_copy(self, parsed):
        ...
        copyoptvals = list(
            map(self.cql_unprotect_value, parsed.get_binding('optvals', ())))
    ...
    def do_expand(self, parsed):
        ...
        """
        self.expand_enabled = SwitchCommand("EXPAND",
                                            "Expanded output").execute(
    ...
    def do_consistency(self, parsed):
        ...
        self.consistency_level = cassandra.ConsistencyLevel.name_to_value[
            level.upper()]
    ...
    def do_serial(self, parsed):
        ...
        if level is None:
            ...
            cassandra.ConsistencyLevel.value_to_name[
    ...
    def get_help_topics(self):
        topics = [t[3:] for t in dir(self) if
                  t.startswith('do_') and getattr(self, t, None).__doc__]
    ...
    def do_help(self, parsed):
        ...
        if not topics:
            ...
            self.print_topics("\nDocumented shell commands:", shell_topics, 15,
                              80)
...
def read_options(cmdlineargs, environment):
    ...
    optvalues.field_size_limit = option_with_default(configs.getint, 'csv',
                                                     'field_size_limit',
                                                     sys.maxsize)
    optvalues.max_trace_wait = option_with_default(configs.getfloat, 'tracing',
                                                   'max_trace_wait',
    ...
    try:
        ...
        parser.error(
            '"%s" is not a valid connect timeout.' % (options.connect_timeout,))
    ...
    try:
        ...
        parser.error(
            '"%s" is not a valid request timeout.' % (options.request_timeout,))
